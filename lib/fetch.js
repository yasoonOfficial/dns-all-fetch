...
//const dnsCache: { [hostName: string]: { addresses: string[], working?: number } } = {};
const dnsCache = {};
...
FetchStream.prototype.runStream = function (url) {
    ...
    if (this.options.asyncDnsLoookup) {
        var dnsCallback = (function (err, addresses) {
            ...
            if (!dnsCache[url_data.urloptions.headers.host] && addresses.length > 1)
                dnsCache[url_data.urloptions.headers.host] = { addresses: addresses };
        ...
        var host = url_data.urloptions.host;
        if (net.isIP(host)) {
            dnsCallback(null, [host]);
        } else if (dnsCache[host] && Number.isInteger(dnsCache[host].working)) {
            dnsCallback(null, [dnsCache[host].addresses[dnsCache[host].working]]);
        } else if (dnsCache[host] && dnsCache[host].addresses.length > this.options._nextIP) {
            dnsCallback(null, [dnsCache[host].addresses[this.options._nextIP++]]);
        } else {
            dns.resolve4(host, dnsCallback);
        }
    } else {
        this._runStream(url_data, url);
    }
...
FetchStream.prototype._runStream = function (url_data, url) {
    ...
    var req = url_data.transport.request(url_data.urloptions, (function (res) {
        var host = url_data.urloptions.headers.host;
        if (dnsCache[host] && !Number.isInteger(dnsCache[host].working)) {
            dnsCache[host].working = this.options._nextIP - 1;
            setTimeout(() => {
    ...
    req.on('error', (function (e) {
        //If connection was not possible, check if we have moar ips
        if (e.code === 'ECONNREFUSED' || e.code === 'ETIMEDOUT') {
            var host = url_data.urloptions.headers.host;
            if (dnsCache[host] && this.options._nextIP < dnsCache[host].addresses.length) {
                req.destroy();
                this.runStream(this.url);
                return;
            }
        }
